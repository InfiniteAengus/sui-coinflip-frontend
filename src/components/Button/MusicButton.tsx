...
const MusicButton = () => {
  ...
  useEffect(() => {
    const savedMuted = JSON.parse(localStorage.getItem('music-muted') || 'false');
    setMuted(savedMuted);
  }, []);
  ...
  return (
    <button
      className={cx(
        ...
        !muted ? 'border-[#8567d8] bg-[#8567d8]' : 'border-[#c75151] bg-[#c75151]',
      )}
      onClick={() => handleMuteClick()}
    >
      {muted ? <MusicOffIcon width={40} /> : <MusicIcon width={34} height={27} />}
    </button>
  );
};
