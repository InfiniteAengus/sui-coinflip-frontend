        ...
        <div className='mt-10 flex flex-col items-center space-y-6'>
          <div className='flex gap-10'>
            {['heads', 'tails'].map((label: string, ind: number) => (
              <button
                className={cx(
                  'trans main-button h-24 w-96 rounded-[36px] text-4xl font-bold text-white shadow-[0px_8px_#242424] active:translate-y-0.5',
          ...
          <div className='grid grid-cols-3 gap-5'>
            {[1, 2, 5, 10, 25, 50].map((amount: number, ind: number) => (
              <button
                className={cx(
                  'trans main-button h-24 w-60 rounded-3xl text-4xl font-bold text-white shadow-[0px_6px_#242424] active:translate-y-0.5',
